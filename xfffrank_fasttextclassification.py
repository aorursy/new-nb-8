...
from keras.layers import Dense, Input, LSTM, Embedding, Dropout, Activation, CuDNNGRU, Conv1D, AveragePooling2D, Lambda
...
print('average length:', sum_len / len(df.question_text))
...
x = Embedding(max_features, embed_size, weights=[embedding_matrix])(inp)  # (?, 100, 300)

x = Lambda(lambda x: expand_dims(x, axis=3))(x)  # (?, 100, 300, 1),  add the number of channel

x = AveragePooling2D(pool_size=(x.shape[1], 1))(x)   # (?, 1, 300, 1)

x = Lambda(lambda x: squeeze(x, axis=1))(x)

x = Lambda(lambda x: squeeze(x, axis=2))(x)  # (?, 300)
...
for thresh in np.arange(0.1, 0.501, 0.01):
    ...
    score = metrics.f1_score(val_y, (pred_glove_val_y>thresh))
...
# If you have additional code, please execute the garbage collection.
