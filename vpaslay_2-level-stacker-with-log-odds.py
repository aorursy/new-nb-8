...
train = pd.read_csv('../input/train.csv', na_values=-1)#, nrows=20000)

test = pd.read_csv('../input/test.csv', na_values=-1)#, nrows=20000)
...
class Ensemble(object):    
    ...
    def fit_predict(self, X, y, T):
        ...
        if num_models==1:
            ...
            F_S_train = F_S_train.rank() / F_S_train.shape[0]
            ...
            stack_res = (res.rank() / res.shape[0]).mean(axis=1)            

            print("2nd level models final score: %.5f" % (roc_auc_score(y, F_S_train.mean(axis=1))))
...
sub.to_csv('2-level_stacked_log_odds.csv', index=False)