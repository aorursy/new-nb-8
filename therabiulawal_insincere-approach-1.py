...
print("Sincere percentage = {0:.2f}".format(insin*100/tot))
...
train_tokenized = tk.texts_to_sequences(train['question_text'].fillna('_##_'))

test_tokenized = tk.texts_to_sequences(test['question_text'].fillna('_##_'))
train['question_text'].apply(lambda x: len(x.split())).plot(kind='hist');

plt.yscale('log');

plt.title('Distribution of train question text length in characters');
max_len = 72
...
class NeuralNet(nn.Module):
    ...
    def forward(self, x):
        ...
        conc = torch.cat((h_lstm_atten, h_gru_atten), 1)
