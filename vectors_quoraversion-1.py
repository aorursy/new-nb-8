...
def train_pred(model, train_X, train_y, val_X, val_y, epochs=2, callback=None):
    for e in range(epochs):
        ...
        best_threshold,best_score = threshold_search(val_y.reshape(-1), pred_val_y.reshape(-1))
        print("Epoch: ", e, "-    Val F1 Score: {:.4f}".format(best_score))

    pred_test_y = model.predict([test_X], batch_size=1024, verbose=0)
    print('=' * 60)
    return pred_val_y, pred_test_y, best_score,best_threshold
...
for idx, (train_idx, valid_idx) in enumerate(splits):
        ...
        pred_val_y, pred_test_y, best_score,best_threshold = train_pred(model, X_train, y_train, X_val, y_val, epochs = 8, callback = [clr,])
        val_thresholds.append(best_threshold)
        train_meta[valid_idx] = pred_val_y.reshape(-1)
        test_meta += pred_test_y.reshape(-1) / len(splits)
        
final_threshold = sum(val_thresholds)/len(val_thresholds)
print("Thresholds used in validation: ",val_thresholds)
print("Final threshold: ",final_threshold)
sub = pd.read_csv('../input/sample_submission.csv')
sub.prediction = test_meta > final_threshold
