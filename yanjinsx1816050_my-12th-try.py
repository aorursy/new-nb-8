...
print (X_train[:,0])
newfeature_from_train=array(vstack((new_feature[0],new_feature[1],new_feature[2],new_feature[3],X_train[:,0],X_train[:,5],X_train[:,9]))).T #拼接完成后做转置
print ('newfeature_from_train',newfeature_from_train.shape)
#将交叉验证获得的label拼接起来(X_test')
predict_from_test_average=array(vstack((new_label_test[0],new_label_test[1],new_label_test[2],new_label_test[3],X_test[:,0],X_test[:,5],X_test[:,9]))).T
...
predDf.to_csv('mypred12.csv',index=False)
print ('over')
