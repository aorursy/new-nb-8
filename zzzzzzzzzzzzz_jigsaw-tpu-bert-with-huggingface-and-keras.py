...
v.to_csv('tpu_bert_hugging_face_keras_submission_val.csv', index=False)
print(roc_auc_score(valid['toxic'], v['toxic']))
v
# sub['toxic'] = np.mean(roc_auc.test_pred, axis=0)
# sub.to_csv('tpu_bert_hugging_face_keras_submission_test.csv', index=False)
sub['toxic'] = model.predict(test_dataset[0], verbose=1)
# sub.to_csv('tpu_bert_hugging_face_keras_submission_test.csv', index=False)
