...
get_ipython().system('mkdir ../train/0')
get_ipython().system('mkdir ../train/0/0')
get_ipython().system('mkdir ../train/1')
...
def process(path, mode, label):
    ...
    for I in range(4):
        ...
        colorImage.save("../%s/%d/%d/%s_%d.png" % (mode, label, label, path, I), "PNG")
        
    colorImage = colorImage.transpose(Image.FLIP_LEFT_RIGHT)

    for I in range(4):
        colorImage = colorImage.transpose(Image.ROTATE_90)
        colorImage.save("../%s/%d/%d/%s_%d.png" % (mode, label, label, path, I+4), "PNG")
    
    colorImage = colorImage.transpose(Image.FLIP_TOP_BOTTOM)
    
    for I in range(4):
        colorImage = colorImage.transpose(Image.ROTATE_90)
        colorImage.save("../%s/%d/%d/%s_%d.png" % (mode, label, label, path, I+8), "PNG")  
...
for I in tqdm_notebook(range(len(values))):
