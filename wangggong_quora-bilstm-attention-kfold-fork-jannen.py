...
class Embed_Layer(nn.Module):
    def __init__(self, embedding_matrix=None, vocab_size=None, embedding_dim=400):
...
class GRU_Layer(nn.Module):
    def __init__(self):
        ...
        self.gru = nn.GRU(input_size=400,
                          hidden_size=gru_len,
                          bidirectional=True)
        '''
...
for i, (train_idx, valid_idx) in enumerate(splits):    
    ...
    kfold_X_features = features_train[train_idx.astype(int)]
    kfold_X_valid_features = features_train[valid_idx.astype(int)]
...
#     model = BiLSTM(lstm_layer=2,hidden_dim=40,dropout=DROPOUT).cuda()
    ...
    for i, (X_batch,) in enumerate(test_loader):
        f = features_test[i * batch_size:(i+1) * batch_size]
        y_pred = model([X_batch,f]).detach()
