...
metadata['year_built'].interpolate(method='pad', inplace=True)
...
# Fill NA first by site id and locally for each day, then for each cite id 
...
for df in [train, test]:
    ...
    for col in cols:
        ...
        df[col].fillna(train.groupby(['site_id', 'date'])[col].transform('mean'), inplace=True)



    for col in cols:

        df[col].fillna(train.groupby('site_id')[col].transform('mean'), inplace=True)
...
idx_to_change = np.logical_and(train['site_id'] == 0, train['meter'] == 'Electricity')

train.loc[idx_to_change, 'meter_reading'] = train.loc[idx_to_change, 'meter_reading'] + np.log(0.2931)

idx_to_change = np.logical_and(test['site_id'] == 0, test['meter'] == 'Electricity')
...
reg = lgb.train(params, lgb_train, num_boost_round=400, valid_sets=[lgb_train, lgb_test], verbose_eval=100)
...
for i in range(0, len(test), step):
    ...
    predictions.extend(np.expm1(reg.predict(test.iloc[i: min(i+step, len(test)), :], num_iteration=reg.best_iteration) - 
                               ...
                               idx_to_change[i: min(i+step, len(test))] * np.log(0.2931)))
