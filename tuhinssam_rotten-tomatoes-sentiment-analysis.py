...
df = pd.read_csv("../input/train.tsv.zip", sep="\t")

df_test = pd.read_csv("../input/test.tsv.zip", sep="\t")
df.shape
df.head()
df_test.head()
...
predicted = pipeline.predict(df_test['Phrase'])
predicted[0:10]
df_submission = pd.DataFrame({"PhraseId": df_test.PhraseId, "Sentiment":predicted})
df_submission.to_csv('submission.csv', index=False)