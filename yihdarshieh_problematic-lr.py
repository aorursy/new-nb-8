...
with strategy.scope():
    ...
    optimizer = tf.keras.optimizers.Adam(learning_rate=lr_schedule.lr_scaled) # not quite sure yet why LR must be scaled up by 8 (otherwise, does not converge the same)
...
def train_step(images, labels):
    ...
    tf.print(" current learning rate = {}".format(lr()))
...
for step, (images, labels) in enumerate(train_dist_ds):
    ...
    if ((step+1) // STEPS_PER_EPOCH) > epoch:
        ...
        print('time: {:0.1f}s'.format(epoch_time),
              ...
              'lr: {:0.4g}'.format(lr_schedule.lrfn(epoch)), flush=True)
