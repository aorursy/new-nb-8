...
target
train_NAS_large = np.load('../input/cats-and-dogs-embedded-data/cats_and_dogs_2/train_NAS_large.npy')

test_NAS_large = np.load('../input/cats-and-dogs-embedded-data/cats_and_dogs_2/test_NAS_large.npy')
train_NAS_large.shape
X_train, X_val, y_train, y_val = train_test_split(train_NAS_large, target, test_size=0.1, random_state=42)

lr = LogisticRegression(C=0.0075, max_iter=10000)
...
test_preds = lr.predict_proba(test_NAS_large)[:,1]
test_preds
