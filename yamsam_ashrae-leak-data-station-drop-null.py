...
print( 'null=', leak0_df.meter_reading.isnull().sum())

leak0_df.dropna(inplace=True)
...
print( 'null=', leak1_df.meter_reading.isnull().sum())

leak1_df.dropna(inplace=True)
...
print( 'null=', leak2_df.meter_reading.isnull().sum())

leak2_df.dropna(inplace=True)
...
print( 'null=', leak4_df.meter_reading.isnull().sum())

leak4_df.dropna(inplace=True)
...
print( 'null=', leak15_df.meter_reading.isnull().sum())

leak15_df.dropna(inplace=True)
...
df.head()
df.to_feather('leak_null.feather')
leak_df = pd.read_feather('leak_null.feather')
leak_df.head()
...
leak_df2 = leak_df[(leak_df.timestamp.dt.year > 2016) & (leak_df.timestamp.dt.year < 2019)]
bmg = leak_df2.groupby(['building_id', 'meter', 'site_id']).meter.count()
bmg.hist(bins=100)
bmg.sort_values()[:20]
len(leak_df[(leak_df.building_id == 1443) & (leak_df.meter == 0)])