...
target
train_InceptionV3 = np.load('../input/cats-and-dogs-embedded-data/cats_and_dogs_1/train_InceptionV3.npy')

test_InceptionV3 = np.load('../input/cats-and-dogs-embedded-data/cats_and_dogs_1/test_InceptionV3.npy')
train_InceptionV3.shape
X_train, X_val, y_train, y_val = train_test_split(train_InceptionV3, target, test_size=0.1, random_state=42)

lr = LogisticRegression(C=0.4, max_iter=10000)
...
test_preds = lr.predict_proba(test_InceptionV3)[:,1]
test_preds
