...
cvp = cross_val_predict(clf, xs, ys, method = 'predict_proba')

#clf.fit(xs, ys)

#pred_ys = clf.predict_proba(xs)[:, 1]

print('average precision', average_precision_score(ys, cvp[:,1]))

pred_mask = cvp[:,1].reshape(train_mask.shape)
