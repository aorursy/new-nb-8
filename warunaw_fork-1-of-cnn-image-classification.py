...
for train_index, test_index in kf.split(X):
    ...
    cnn_model.add(Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=(train_avg_shape,train_avg_shape,3), padding='same'))
...
history = cnn_model.fit(X, Y, epochs=epochs, batch_size=batch_size, validation_split = 0.1)    
...
for k,v in class_name_numb.items():

    inverse_label_map[v] = k
y_predicted = cnn_model.predict(X_test)
y_predicted= pd.DataFrame(y_predicted)
y_predicted = y_predicted.idxmax(axis=1).values
...
pred.head()
pred.to_csv('predictionsModel7.csv',index = None, header=True)