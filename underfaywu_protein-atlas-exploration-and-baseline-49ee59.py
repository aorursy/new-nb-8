...
test = pd.read_csv("../input/human-protein-atlas-image-classification/sample_submission.csv")
test.head()
...
files = listdir("../input/human-protein-atlas-image-classification/test")
for n in range(10):
    print(files[n])
L = []
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        return X, y
self.labels.loc[self.labels.Id==identifier][self.target_wishlist].values
class PredictGenerator:
    ...
    def predict(self, model):
        ...
        return y
predict_generator_test = PredictGenerator(L, preprocessor, test_path)
...
predict_generator = PredictGenerator(test.Id.values, preprocessor, test_path)
...
feature = "Nucleoplasm"
plt.figure(figsize=(20,5))
sns.distplot(baseline_proba_predictions[feature].values, color="Purple")
...
improved_proba_predictions.head()
